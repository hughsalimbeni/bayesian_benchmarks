...
class Dataset(object):
    ...
    def download(self):
        ...
        is_zipped = np.any([z in self.url for z in [".zip", ".tar"]])
        is_gzipped = ".gz" in self.url

        if is_zipped or is_gzipped:
...
class WilsonDataset(Dataset):
    base_url = (
        "https://media.githubusercontent.com/media/treforevans/uci_datasets/master/"
    )
    ...
    @property
    def url(self):
        return self.base_url + self.name[len("wilson_") :] + "/data.csv.gz"
